...
public class DownloadAtomFeedService extends IntentService {
    ...
    public static Intent makeIntent(Context context, int requestCode, Uri url, Handler
        ...
        return new Intent (context,DownloadAtomFeedService.class)
                .setData (url)
    ...
    public void onHandleIntent(Intent intent) {
        ...
        try {
            entries = downloadAtomFeed (uri.toString ());
        } catch (Exception e) {
            e.printStackTrace ();
        }
        ...
        sendEntries (messenger, (ArrayList<Entry>) entries,uri,requestCode);
    }
    ...
    private Message makeReplyMessage(ArrayList<Entry> entries, Uri url, int requestCode) {
        ...
        bundle.putParcelableArrayList (ENTRY_ARRAY_KEY,entries);
...
//        }
        message.arg1 = entries.size () <= 0 ? Activity.RESULT_CANCELED : Activity.RESULT_OK;
