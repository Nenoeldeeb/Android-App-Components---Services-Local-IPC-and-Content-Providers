...
public class MainActivity extends CustomLoggingActivityBase implements ServiceResult {
    ...
    private void startDownload(Uri url) {
        ...
        Intent intent = DownloadAtomFeedService.makeIntent (getApplicationContext (),REQUEST_YOUTUBE_ENTRIES,url,mServiceResultHandler);
    ...
    public void onServiceResult(int requestCode, int resultCode, Bundle data) {
        ...
        // Log to both the on-screen & logcat logs the requestUri from the data.
            ...
            Log.d (TAG,"There are " + entries.size () + "entrie(s) downloaded.");

            Toast.makeText (getApplicationContext (),"There are " + entries.size () + "entrie(s) downloaded.",Toast.LENGTH_LONG).show ();
