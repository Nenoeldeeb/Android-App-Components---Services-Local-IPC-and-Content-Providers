...
public class FeedProvider extends ContentProvider {
	...
	synchronized public String getType (@NonNull Uri uri) {
		...
		switch (mUriMatcher.match (uri)) {
			...
			default:
				...
				throw new UnsupportedOperationException ("URL: " + uri + " is not supported.");
		}
	}
	...
	synchronized public Cursor query (@NonNull final Uri uri, final String[] projection,
		...
		return query (uri,FeedContract.Entry.TABLE_NAME,projection,modifiedSelection,selectionArgs,updatedSortOrder);
	}
	...
	synchronized private Cursor query (final Uri uri, final String tableName,
		...
		sql.setTables (tableName);

		// return the builder.query(....) result, after passing the appropriate values.
		return sql.query (mDB.getDB (), projection, selection, selectionArgs, null, null, sortOrder);
	}
	...
	synchronized public Uri insert (@NonNull Uri uri, ContentValues assignedValues) {
		...
		switch (mUriMatcher.match (uri)) {
			...
			case ALL_ENTRY_ROWS:
				...
				assignedValues.remove ("ID");

				// insert the ContentValues into the database adapater instance and store the
				// returned rowID.
				long rowID = mDB.insert (FeedContract.Entry.TABLE_NAME, assignedValues);
	...
	synchronized public int delete (@NonNull Uri uri,
		...
		switch (mUriMatcher.match (uri)) {
			...
			case SINGLE_ENTRY_ROW:
				...
				if (whereArgs == null) {
					modifiedWhereClause = "ID = last path segement";
				} else {
					modifiedWhereClause = " AND " + "ID = last path segement";
				}
	...
	private int deleteAndNotify (final Uri uri, final String tableName,
		...
		int row = mDB.delete (tableName, whereClause, whereArgs);
	...
	synchronized public int update (@NonNull Uri uri,
		...
		switch (mUriMatcher.match (uri)) {
			...
			case SINGLE_ENTRY_ROW:
				...
				if (whereClause == null) {
					modifedWhereClause = "_ID = last path segement";
				} else {
					modifedWhereClause = " AND " + "_ID = last path segement";
				}
			...
			case ALL_ENTRY_ROWS:
				return updateAndNotify (uri,FeedContract.Entry.TABLE_NAME,values,modifedWhereClause,whereArgs);
	...
	private int updateAndNotify (@NonNull final Uri uri, final String tableName,
		...
		int row = mDB.update (tableName,values,whereClause,whereArgs);
